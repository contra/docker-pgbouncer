...
RUN \
  # Download
  apk --update add autoconf autoconf-doc automake udns udns-dev curl gcc libc-dev libevent libevent-dev libtool make mandoc openssl-dev pkgconfig postgresql-client && \
  ...
  cd /tmp && \
  rm -rf /tmp/pgbouncer*  && \
  apk del --purge autoconf autoconf-doc automake udns-dev curl gcc libc-dev libevent-dev libtool make mandoc libressl-dev pkgconfig
